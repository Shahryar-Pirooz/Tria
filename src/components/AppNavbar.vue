...
<template>
  ...
  >
    <div class="max-w-5xl w-full mx-auto flex flex-row justify-between">
      ...
      <div class="hidden flex-1 text-content-muted md:block">
        <ul class="flex flex-row flex-nowrap justify-center h-full space-x-4 items-center">
          <li
            class="select-none hover:cursor-pointer hover:text-content active:scale-95"
            v-for="item in items"
            :key="item.id"
          >
            <a :href="item.link">
      ...
      <div @click="openMenuHandler" class="h-full md:hidden">
        <v-icon v-if="isMenuOpen" name="ri-close-line" scale="1.2" />

        <v-icon v-else name="ri-menu-3-line" scale="1.2" />
      </div>
    </div>
  </header>
  <div
    class="flex-col scroll inset-x-0 top-10 z-50 py-8 px-4 flex transition-all duration-300 overflow-hidden md:hidden"
    :class="isMenuOpen ? 'flex' : 'hidden'"
  >
    <div class="flex-1 text-content-muted">
...
.scroll {
  position: fixed;
  background: oklch(from var(--color-page) l c h / 0.7);
  backdrop-filter: blur(20px);
  -webkit-backdrop-filter: blur(20px);
  border-bottom: 1px solid oklch(from var(--color-contrast) l c h / 0.05);
}
</style>
